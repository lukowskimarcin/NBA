...
class GraphRepository:
    ...
    def find_all(self) -> list[NBAGraph]:
        ...
        for row in rows:
            graph = NBAGraph(
                ...
                row.initial_wps,
                row.current_wps,
                row.best_action_path,
                row.is_action_performed,
                row.is_active,
            )
            graphs.append(graph)
        conn.close()
    ...
    def save(self, graph: NBAGraph):
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM NBA WHERE client_id = ? AND dlum_id = ?",
            (graph.client_id, graph.dlum_id),
        )
        ...
        if existing_record_count > 0:
            cursor.execute(
                "UPDATE NBA SET initial_wps = ?, current_wps = ?, best_action_path = ?, is_action_performed = ?, is_active = ? WHERE client_id = ? AND dlum_id = ?",
                (
                    graph.initial_wps,
                    graph.current_wps,
                    best_action_path,
                    graph.is_action_performed,
                    graph.is_active,
                    graph.client_id,
                    graph.dlum_id,
                ),
            ...
            cursor.execute(
                "INSERT INTO NBA (client_id, dlum_id, initial_wps, current_wps, best_action_path, is_action_performed, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
                    graph.client_id,
                    graph.dlum_id,
                    graph.initial_wps,
    ...
    def findById(self, client_id, dlum_id) -> NBAGraph:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM NBA WHERE client_id = ? AND dlum_id = ?",
            (client_id, dlum_id),
        )
        row = cursor.fetchone()
